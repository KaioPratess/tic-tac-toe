...
events.subscribe('multiplayer', events.events, () => {
  const multiplayerMode = (function () {
    ...
    const multiplayerModeDiv = document.querySelector('.multiplayer-mode');
    const playerInput = document.querySelectorAll(
      '.multiplayer-mode input[type="text"]',
    );
    ...
    function _createPlayer() {
      const player1Name =
        playerInput[0].value.charAt(0).toUpperCase() +
        playerInput[0].value.slice(1, playerInput[0].value.length);
      const player2Name =
        playerInput[1].value.charAt(0).toUpperCase() +
        playerInput[1].value.slice(1, playerInput[1].value.length);
...
events.subscribe('solo', events.events, () => {
  const soloMode = (function () {
    ...
    function _createPlayer() {
      const playerName =
        playerInput.value.charAt(0).toUpperCase() +
        playerInput.value.slice(1, playerInput.value.length);
    ...
    function _createMachine() {
      const nameDisplay1 = document.querySelector(
        '.score-panel ul li:first-of-type p',
      );
      const nameDisplay2 = document.querySelector(
        '.score-panel ul li:last-of-type p',
      );
